...
// import Order from './Men'
  ...
  handlebillcardnumberChange = (event) => {
    this.props.dispatch({ type: 'billcardnumber', payload: event.target.value });
  };

  handlebillcardexpmonthChange = (event) => {
    this.props.dispatch({ type: 'billcardexpmonth', payload: event.target.value });
  };

  handlebillcardexpyearChange = (event) => {
    this.props.dispatch({ type: 'billcardexpyear', payload: event.target.value });
  };

  handlebillcardexpcvvChange = (event) => {
    this.props.dispatch({ type: 'billcardexpcvv', payload: event.target.value });
  };
  ...
  <div class="col-75">
    <div class="container ccout">
      <form>
        ...
        <div class="row">
          <div class="col-50">
            ...
            <input type="text" id="fname" name="fullname" placeholder="Full Name" 
                    ...
                    onChange={(event) => this.handlebillnameChange(event)} required />
            <label for="email"><i class="fa fa-envelope"></i> Email</label>
            <input type="text" id="email" name="email" placeholder="example@example.com" 
                    value={this.props.billemail}
                    onChange={(event) => this.handlebillemailChange(event)} required />
            <label for="adr"><i class="fa fa-address-card-o"></i> Address</label>
            <input type="text" id="adr" name="address" placeholder="address" 
                    value={this.props.billaddress}
                    onChange={(event) => this.handlebilladdressChange(event)} required />
            <label for="city"><i class="fa fa-institution"></i> City</label>
            <input type="text" id="city" name="city" placeholder="city" 
                    value={this.props.billcity}
                    onChange={(event) => this.handlebillcityChange(event)} required />
            ...
            <div class="row">
              <div class="col-50">
                ...
                <input type="text" id="state" name="state" placeholder="state" 
                    ...
                    onChange={(event) => this.handlebillstateChange(event)} required />
              </div>
              <div class="col-50">
                <label for="zip">Zip</label>
                <input type="text" id="zip" name="zip" placeholder="zip" 
                    value={this.props.billzip}
                    onChange={(event) => this.handlebillzipChange(event)} required /> 
              </div>
            </div>
          </div>
          ...
          <div class="col-50">
            ...
            <input type="text" id="cname" name="cardname" placeholder="Card name" 
                    ...
                    onChange={(event) => this.handlebillcardnameChange(event)} required />
            <label for="ccnum">Credit card number</label>
            <input type="text" id="ccnum" name="cardnumber" placeholder="1111-2222-3333-4444" 
                    value={this.props.billcardnumber}
                    onChange={(event) => this.handlebillcardnumberChange(event)} required />
            <label for="expmonth">Exp Month</label>
            <input type="text" id="expmonth" name="expmonth" placeholder="September" 
                    value={this.props.billcardexpmonth}
                    onChange={(event) => this.handlebillcardexpmonthChange(event)} required />
            <div class="row">
              <div class="col-50">
                <label for="expyear">Exp Year</label>
                <input type="text" id="expyear" name="expyear" placeholder="2018" 
                    value={this.props.billcardexpyear}
                    onChange={(event) => this.handlebillcardexpyearChange(event)} required />
              </div>
              <div class="col-50">
                <label for="cvv">CVV</label>
                <input type="text" id="cvv" name="cvv" placeholder="352" 
                    value={this.props.billcardexpcvv}
                    onChange={(event) => this.handlebillcardexpcvvChange(event)} required />
              </div>
            </div>
          </div>
        ...
        <button class="btn btn-success" disabled={!this.validatecheckoutaddressDetails()} onClick={() => {
    this.showBill();
  }}>
